...
function Get-R0UnexpectedGitCount {
    ...
    $unexpected = @($status | Where-Object { $AllowedStatusLines -notcontains $_ })
    $missingAllowed = @($AllowedStatusLines | Where-Object { $status -notcontains $_ })
    return ($unexpected.Count + $missingAllowed.Count)
}
