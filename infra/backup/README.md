...
현재 head revision ``20260803_0014_recipient_plan_notification``까지 지원한다.
...
- `scripts/restore-drill.ps1`은 manifest를 검증하고 이름이 `_review`로 끝나는 새
  ...
  ``RECIPIENT_PLAN_NOTIFICATION_DB_POSTCHECK_OK``)가 출력되어야만 성공으로
  간주한다. 파일 hash까지 통과한 뒤, 별도 보존 옵션이 없으면 검증 DB와
  파일을 제거한다.
